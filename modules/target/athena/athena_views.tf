...
# =============================================================================
#
# Creates a named query per account that generates a view mapping the
# slash-delimited column names (e.g. "lineitem/unblendedcost") to the
# underscore format expected by the default Grafana "Athena Cost and Usage
# Report" dashboard (e.g. line_item_unblended_cost).
#
# To use: Run the named query in Athena console, then set CUR_Table to
# {account}_standard in the Grafana dashboard.
# =============================================================================

resource "aws_athena_named_query" "standard_view" {
  for_each = var.source_accounts

  name        = "create_${each.key}_standard_view"
  database    = aws_glue_catalog_database.cur_database.name
  workgroup   = aws_athena_workgroup.cur_analysis.name
  description = "Creates a standard CUR view for ${each.key} compatible with the default Grafana Athena CUR dashboard"

  query = <<-EOQ
    CREATE OR REPLACE VIEW ${replace(each.key, "-", "_")}_standard AS
    SELECT
      "identity/lineitemid" AS identity_line_item_id,
      "identity/timeinterval" AS identity_time_interval,
      "bill/invoiceid" AS bill_invoice_id,
      "bill/invoicingentity" AS bill_invoicing_entity,
      "bill/billingentity" AS bill_billing_entity,
      "bill/billtype" AS bill_bill_type,
      "bill/payeraccountid" AS bill_payer_account_id,
      CAST(TRY(from_iso8601_timestamp("bill/billingperiodstartdate")) AS timestamp) AS bill_billing_period_start_date,
      CAST(TRY(from_iso8601_timestamp("bill/billingperiodenddate")) AS timestamp) AS bill_billing_period_end_date,
      "lineitem/usageaccountid" AS line_item_usage_account_id,
      "lineitem/lineitemtype" AS line_item_line_item_type,
      CAST(TRY(from_iso8601_timestamp("lineitem/usagestartdate")) AS timestamp) AS line_item_usage_start_date,
      CAST(TRY(from_iso8601_timestamp("lineitem/usageenddate")) AS timestamp) AS line_item_usage_end_date,
      "lineitem/productcode" AS line_item_product_code,
      "lineitem/usagetype" AS line_item_usage_type,
      "lineitem/operation" AS line_item_operation,
      "lineitem/availabilityzone" AS line_item_availability_zone,
      "lineitem/resourceid" AS line_item_resource_id,
      TRY_CAST("lineitem/usageamount" AS DOUBLE) AS line_item_usage_amount,
      TRY_CAST("lineitem/normalizationfactor" AS DOUBLE) AS line_item_normalization_factor,
      TRY_CAST("lineitem/normalizedusageamount" AS DOUBLE) AS line_item_normalized_usage_amount,
      "lineitem/currencycode" AS line_item_currency_code,
      TRY_CAST("lineitem/unblendedrate" AS DOUBLE) AS line_item_unblended_rate,
      TRY_CAST("lineitem/unblendedcost" AS DOUBLE) AS line_item_unblended_cost,
      TRY_CAST("lineitem/blendedrate" AS DOUBLE) AS line_item_blended_rate,
      TRY_CAST("lineitem/blendedcost" AS DOUBLE) AS line_item_blended_cost,
      "lineitem/lineitemdescription" AS line_item_line_item_description,
      "lineitem/taxtype" AS line_item_tax_type,
      "lineitem/legalentity" AS line_item_legal_entity,
      "product/productname" AS product_product_name,
      "product/availabilityzone" AS product_availability_zone,
      "product/instancetype" AS product_instance_type,
      "product/instancefamily" AS product_instance_family,
      "product/location" AS product_location,
      "product/locationtype" AS product_location_type,
      "product/operatingsystem" AS product_operating_system,
      "product/operation" AS product_operation,
      "product/productfamily" AS product_product_family,
      "product/region" AS product_region,
      "product/regioncode" AS product_region_code,
      "product/servicecode" AS product_servicecode,
      "product/servicename" AS product_servicename,
      "product/tenancy" AS product_tenancy,
      "product/usagetype" AS product_usagetype,
      "product/vcpu" AS product_vcpu,
      "product/memory" AS product_memory,
      "product/storage" AS product_storage,
      "product/networkperformance" AS product_network_performance,
      "product/volumetype" AS product_volume_type,
      TRY_CAST("pricing/publicondemandcost" AS DOUBLE) AS pricing_public_on_demand_cost,
      TRY_CAST("pricing/publicondemandrate" AS DOUBLE) AS pricing_public_on_demand_rate,
      "pricing/term" AS pricing_term,
      "pricing/unit" AS pricing_unit,
      TRY_CAST("reservation/effectivecost" AS DOUBLE) AS reservation_effective_cost,
      TRY_CAST("savingsplan/savingsplaneffectivecost" AS DOUBLE) AS savings_plan_savings_plan_effective_cost
    FROM ${replace(each.key, "-", "_")}
  EOQ
}
